import { BASE_POSITIONS, HOME_ENTRANCE, HOME_POSITIONS, PLAYERS, SAFE_POSITIONS, START_POSITIONS, STATE, TURNING_POINTS, WIN} from './coordinates.js';
...
export class Ludo {
  ...
  /*  listenResetClick() {
    ...
    onPieceClick(event) {
       ...
       if(!target.classList.contains('piece') || !target.classList.contains('pieceHighlight')) {
            return;
        } 
    ...
    movePiece(player, piece, moveBy) {
        ...
        PLAYERS.splice(this.turn, 1);
                ...
                if(isKill || this.diceValue === 6 || this.currentPositions[player][piece] === HOME_POSITIONS[player]) {
                    this.state = STATE.DICE_NOT_ROLLED;
                    return;
                }
    ...
    checkForKill(player, piece) {
        ...
        [0, 1, 2, 3].forEach(piece => {
            ...
            if(currentPosition === opponentPosition && !SAFE_POSITIONS.includes(currentPosition)) {
                this.setPiecePosition(opponent[i], piece, BASE_POSITIONS[opponent[i]][piece]);
                kill = true
            }
        });
        }
