...
activePlayerBase.classList.remove('baseHighlight');
        ...
        const baseHighlighter = document.querySelector(`[player-id="${player}"].home`)
       setTimeout(() => { 
baseHighlighter.classList.add('baseHighlight')
        }, 800);
        const container = document.querySelector(`[player-id="${player}"].container`);
        
       setTimeout(() => {                
    container.appendChild(diceButtonElement)
          
        if (document.querySelector(`[player-id="P1"].container`).contains(diceButtonElement)) {
    diceButtonElement.style.display = "block";
            for (let i = 0; i < 6; i++) {
                face[i].style.background = "#b30007";
             }
        } else if (document.querySelector(`[player-id="P2"].container`).contains(diceButtonElement)) {
    diceButtonElement.style.display = "block";
            for (let i = 0; i < 6; i++) {
                face[i].style.background = "#2a8400";
             }
        } else if (document.querySelector(`[player-id="P3"].container`).contains(diceButtonElement)) {
    diceButtonElement.style.display = "block";
            for (let i = 0; i < 6; i++) {
                face[i].style.background = "#1c47a6";
             }
        } else if (document.querySelector(`[player-id="P4"].container`).contains(diceButtonElement)) {
    diceButtonElement.style.display = "block";
            for (let i = 0; i < 6; i++) {
